...
###########################################################
DROP DATABASE IF EXISTS gregg_db;
CREATE DATABASE gregg_db;
...
SELECT DISTINCT c.name, CONCAT(c.name, " ", a2.address) AS "Below 50 Accounts" FROM customers c
JOIN accounts_customers ac on c.customer_id = ac.customer_id
JOIN accounts a on ac.account_id = a.account_id
...
JOIN address ad on customers.address_id = ad.address_id
WHERE ad.state = "TX";
